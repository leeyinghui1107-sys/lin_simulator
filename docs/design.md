...
主程序只依赖本地 SQLite。设备模拟运行时不访问远程数据库。
...
| `model.go` | `PortInfo` 状态锁、重试计数、连接计数、命令映射、命令长度集合和 TCP 前缀匹配索引 |
| `server.go` | TCP 监听、指数退避重试、连接限制、连接处理和优雅关闭 |
| `utils.go` | 本机 IP 自动识别、`-ip` 校验、字节转十六进制 |
| `build.ps1` | Windows 下执行的多平台发布构建脚本 |
...
5. 校验 SQLite 文件存在并打开。
6. `LoadPortMap(db, bindHost)` 加载端口和命令。
7. `signal.NotifyContext` 接管 `SIGINT` / `SIGTERM`。
8. `Start(ctx, pm, delay, maxConn)` 启动所有端口。
...
- `delay < 0`、空命令、非法十六进制命令/返回值、超过 64KB 的命令都会导致启动加载失败。
- `port <= 0` 或 `port > 65535` 的记录不会被加载。
...
- 每个连接维护独立的命令轮转索引。
- 每次读取后重置 5 分钟读超时。
- 读取的是 TCP 字节流，连接内维护缓冲区；按已知命令做最长前缀匹配，支持拆包/粘包。
- 当前缓冲仍是某个命令前缀时继续等待；无法形成任何命令前缀时丢弃 1 字节并继续同步，Debug 日志记录。
- 连接命令缓冲区上限为 64KB；超过上限时关闭该连接。
